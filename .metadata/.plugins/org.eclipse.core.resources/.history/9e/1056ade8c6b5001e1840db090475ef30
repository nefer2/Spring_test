...
document.addEventListener('click', (e) => {
  ...
  if (e.target.id == 'moreBtn') {
    ...
    //수정 => cno,writer,content 을 위한 cno,writer data-로 달아주기
...
async function removeCommentToServer(cnoVal, writerVal) {
  try {
    const url = "/comment/delete/" + cnoVal + "/" + writerVal;
    const config = {
      method: "delete"
    };
    const resp = await fetch(url, config);
